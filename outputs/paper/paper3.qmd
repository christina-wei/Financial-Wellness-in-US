...
The 6 band social class survey expands the middle class into three categories: lower middle class, middle class, and upper middle class. Comparing the distribution between the two social class variables (@fig-socialclass), there are movements across the bands.
...
survey_data_2021 |>
  ...
  filter (social_class == "Upper Class")
```
...
survey_data_2021 |>
  filter(!is.na(income), !is.na(social_class)) |>
  ggplot(aes(x = social_class, group = income)) +
...
### (Economic) The Rich are getting Richer, and the Poor are getting Poorer
